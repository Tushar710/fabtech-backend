...
// Add/Update hourly activity
router.post('/activity', auth, async (req, res) => {
  try {
    const employeeId = req.user.id || req.user._id;
    const { hour, timeSlot, activity, description, status, date } = req.body;
    
    if (hour === undefined || !timeSlot || !activity) {
      return res.status(400).json({
        success: false,
        message: 'Hour, timeSlot, and activity are required'
      });
    }
    ...
    console.log('📝 Adding activity for IST date:', {
    ...
    // Check if activity for this hour already exists
    const existingIndex = report.hourlyActivities.findIndex(a => a.hour === hour);
    
    if (existingIndex >= 0) {
      // Update existing activity
      report.hourlyActivities[existingIndex] = {
        hour,
        timeSlot,
        activity,
    ...
    // Sort by hour
    report.hourlyActivities.sort((a, b) => a.hour - b.hour);
    
    // Calculate total hours worked
    report.totalHoursWorked = report.hourlyActivities.length;
...
// Delete hourly activity
router.delete('/activity/:reportId/:hour', auth, async (req, res) => {
  try {
    const employeeId = req.user.id || req.user._id;
    const { reportId, hour } = req.params;
